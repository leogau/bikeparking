...
def get_nearest_locations(lat, lon, search_distance):
    ...
    sfdata_query_params = {
        '$where': "status_detail = 'INSTALLED' AND racks_installed > 0 AND within_box(coordinates, " + north + ", " + east + ", " + south + ", " + west + ")",
...
@app.route('/api/v1.0/directions', methods=['GET'])
def get_directions():
    latitude = float(request.args.get('latitude'))
    longitude = float(request.args.get('longitude'))
    
    initial_search_distance = 0.1
    result = get_nearest_locations(latitude, longitude, initial_search_distance)
